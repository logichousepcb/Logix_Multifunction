...
void handleRoot () {
static const char PROGMEM ROOT_HTML_PRE[] = R"rawliteral( 
...
static const char PROGMEM ROOT_HTML_PREB[] = R"rawliteral( 
  <p>
    <form id=but6 style="display: block;"action='http://
)rawliteral";
static const char PROGMEM ROOT_HTML_POSTB[] = R"rawliteral(     
      'method='get'>
      <button>DASHBAORD</button>
    </form>
  </p>
)rawliteral";
static const char PROGMEM ROOT_HTML_POST[] = R"rawliteral( 
  <p>
...
String html_outline = ROOT_HTML_PRE;

if (atoi(conf.getValue("dash-bt")) == 1) {  // only add in the dashboard button option checked then use address assigned from config file
  html_outline += ROOT_HTML_PREB;
  html_outline += conf.getValue("dash-link");                 
  html_outline += ROOT_HTML_POSTB;
}

html_outline += ROOT_HTML_POST;
