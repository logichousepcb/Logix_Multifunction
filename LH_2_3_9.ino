...
const char* UpdateserverIndex = "<form method='POST' action='/update' enctype='multipart/form-data'><input type='file' name='update'><input type='submit' value='Update'></form>";
...
// THIS CONNECTS TO MQTT WITHOUT BLOCKING MAIN LOOP - START  
  if (!client.connected()) {
    ...
    if (display_connected) {display_status();};
    client.loop();
    if (freshboot && (atoi(conf.getValue("mqtt-ad"))== 1)) {DLINE2 = "  READY";AD_all_entities ();DLINE2 = "MQTT->AD";display_status();freshboot = false;};  // on a fresh boot auto discover 
