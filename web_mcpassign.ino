...
String html_pinname_front = " <form style=\"text-align: center;\" action=\"save\"/\" autocomplete=\"off\"> ";
...
String html_class_select5  = ">Relay </option> <option value=\"Light\"";
String html_class_select6  = ">Light</option> <option value=\"Lock\"";
String html_class_selectc  = ">Lock</option> </select><br>";
...
void handleActiveChipAssignPage() {
  ...
  for (i = 0; i<8; i++) {
    ...
    if (activechip == '0') {            // DO THIS IF CHIP IS NOT ACTIVE (NOTHING)
        html_header += html_chip_inactive;
    }
  }
...
// if its a class field do this     
      if (chipn == 'c') {  
       ...
       if (argfn == "Light") {pinassign = '5';}; 
       if (argfn == "Lock") {pinassign = '6';}; 
